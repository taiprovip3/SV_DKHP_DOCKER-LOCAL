...
public interface ChuongTrinhKhungRepository extends CrudRepository<ChuongTrinhKhung, Long> {
	...
	@Query(value = "SELECT * FROM chuong_trinh_khung WHERE ma_mon_hoc = ?1 AND ma_sinh_vien = ?2 LIMIT 1", nativeQuery = true)
    public ChuongTrinhKhung getStudentPatternByDependSubjectId(long dependSubjectId, long studentId);
}
