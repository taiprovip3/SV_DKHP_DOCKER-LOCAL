...
public interface MonHocRepository extends CrudRepository<MonHoc, Long> {
	...
	// @Query(value = "SELECT mh.* FROM khoa_hoc kh JOIN hoc_ky hk ON kh.ma_hoc_ky = hk.ma_hoc_ky JOIN mon_hoc mh ON hk.ma_hoc_ky = mh.ma_hoc_ky WHERE kh.ma_khoa_hoc = ?1 AND mh.ma_mon_hoc NOT IN (SELECT ma_mon_hoc FROM chuong_trinh_khung WHERE ma_sinh_vien = ?2)", nativeQuery = true)
	...
	public List<MonHoc> getSubjectsByCourseId(long courseId, long majorId);
	...
	@Query(value = "SELECT DISTINCT mh.* FROM lop_hoc_phan lhp JOIN mon_hoc mh ON lhp.ma_mon_hoc = mh.ma_mon_hoc WHERE lhp.ma_khoa_hoc = ?1 AND mh.ma_nganh = ?2 AND mh.ma_mon_hoc NOT IN (SELECT ma_mon_hoc FROM chuong_trinh_khung WHERE ma_sinh_vien = ?3)", nativeQuery = true)
    public List<MonHoc> getSubjectsUnitClassOpen(long courseId, long majorId, long studentId);
}
