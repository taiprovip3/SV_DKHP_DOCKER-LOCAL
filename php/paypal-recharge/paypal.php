...
if(!isset($_POST['maSinhVien'])) {
    ...
    if($maSinhVien <= 0) {
        ...
        return;
            ...
            $maSinhVien = $_POST['maSinhVien'];
            $unDebtIds = 'Array';
            $url = "http://localhost:8080/api/payment/createTransaction/".$maSinhVien."/".$total_payment."/".$unDebtIds."/EMPTY";
            $maGiaoDich = file_get_contents($url);
            $orderType = 'PAYPAL - WALLET';
            $callBackUrl = 'http://localhost:4000/student/recharge/callback?extraData='.$maGiaoDich.'&resultCode=0&amount='.$total_payment.'&orderType='.$orderType.'';    
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
paypal.Buttons({
    ...
    onApprove: (data, actions) => {
        let callbackUrl = "http://localhost:4000/student/recharge/callback?resultCode=1006";
        callbackUrl = '<?php echo $callBackUrl ?>';
        return actions.redirect();
    },
    onCancel: function(data) {
        window.location.href = "http://localhost:4000/student/recharge/callback?resultCode=1006";
    }
